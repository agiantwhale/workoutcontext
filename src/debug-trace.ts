...
export function registerDebugTraceTool(
  ...
  server.tool(
    "debug_trace",
    [
      "File a structured bug report when the user is dissatisfied with the result of a tool call or sequence of tool calls. The report becomes a GitHub Issue in a private feedback repo and is reviewed by the maintainer to improve tool descriptions, schemas, and behavior.",
      "",
      "WHEN TO USE:",
      "- The user explicitly asks to file feedback / report a bug / submit a debug trace.",
      "- The user expresses clear dissatisfaction with what just happened (e.g. \"that's wrong\", \"this isn't what I asked for\", \"the tool gave me bad data\") AND you've already tried to recover and failed.",
      "",
      "DO NOT use this tool for routine errors that you can recover from yourself, or as a substitute for retrying a tool call with corrected arguments.",
      "",
      ...
      "",
      "RATE LIMIT: one report per hour per user. If rate-limited, do not retry; tell the user when they can file the next one.",
      "",
