...
const SERVER_INSTRUCTIONS = [
  `This server provides AI-driven recovery analysis for athletes by connecting their Intervals.icu, Hevy, Strava, Oura, and Withings accounts. Tools are registered per-user based on which providers they've connected. Server build: \`${GIT_COMMIT_SHORT}\`.`,
  "",
  ...
  "",
  `Build-drift detection: every tool response includes the current server build hash in \`_meta.server_build\`. If you ever observe a value different from \`${GIT_COMMIT_SHORT}\` (the build that started this session), the worker was redeployed mid-session and your cached tool schemas may be stale — stop calling tools, tell the user "the server was redeployed; please start a new conversation to refresh the tools list", and wait for them to do so.`,
].join("\n");
